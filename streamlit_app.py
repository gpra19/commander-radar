...
# --- SISTEM MEMORI (CACHE) AGAR LOADING SUPER CEPAT ---
@st.cache_data(ttl=300) # Simpan data radar selama 5 menit
def tarik_data_radar(ticker):
    return yf.download(ticker, period="3mo", interval="1d", progress=False)

@st.cache_data(ttl=60) # Simpan harga live portofolio selama 1 menit
        ...
        if not ticker_input:
            ...
            try:
                with st.spinner('Menarik data dari satelit...'):
                    ...
                    # Menggunakan fungsi Cache di sini
                    ...
                    if data.empty:
                        ...
                        with tab1:
                            ...
                            tanggal_min, tanggal_max = hist_data['Date'].min(), pd.Series(tgl_pred).max()
                        ...
                        with tab2:
                            ...
                            for col in df_raw.columns: df_raw[col] = df_raw[col].apply(format_rp)
    ...
    try:
        ...
        with st.spinner("Menghitung valuasi portofolio..."):
            for index, row in df_porto.iterrows():
                ...
                # Menggunakan fungsi Cache agar tidak loading lama
